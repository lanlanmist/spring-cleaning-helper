...
class OCRApp:
    ...
    def setup_global_esc_listener(self):
        """监听全局 Esc（不拦截按键），用于前台不在本程序时的紧急停止。"""
        ...
        try:
            ...
            self._keyboard_hotkey = None
            self._keyboard_module = None
            self.log(f"全局 Esc 监听启用失败：{e}。Esc 仅在本窗口聚焦时生效。")

    def on_global_esc_pressed(self):
        if self.is_running and not self.stop_requested:
            self.log("检测到全局 Esc，执行紧急停止。")
            self.stop_task()

    def on_close(self):
        if self._keyboard_module is not None and self._keyboard_hotkey is not None:
